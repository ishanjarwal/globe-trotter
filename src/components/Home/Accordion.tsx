...
const Faq3 = () => {
  const heading = "FAQs";
  const description = "Some frequently asked questions about us";

  return (
    <section className="py-32 bg-black">
      <div className="container space-y-16">
        <div className="mx-auto flex max-w-3xl flex-col text-left md:text-center relative z-10">
          <BgShape width="80%" right="0" height="200px"></BgShape>
          <h2 className="mb-3 text-3xl font-semibold md:mb-4 lg:mb-6 lg:text-4xl text-accent">
            {heading}
          </h2>
          ...
          {items.map((item) => (
            <AccordionItem key={item.id} value={item.id}>
              <AccordionTrigger className="transition-opacity duration-200 hover:no-underline">
                <div className="font-medium sm:py-1 lg:py-2 lg:text-lg  text-accent">
                  {item.question}
                </div>
              </AccordionTrigger>
              <AccordionContent className="sm:mb-1 lg:mb-2">
                <div className="lg:text-lg  text-accent">
                  {item.answer}
                </div>
              </AccordionContent>
            </AccordionItem>
